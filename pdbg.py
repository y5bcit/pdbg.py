        ...
        if len(self.prevline) > 0:
            ...
            for key in changedvars.keys():               
                if(key in self.variable) or len(self.variable) == 0:
                    #print(key,changedvars[key])
                    print("[Debug]", self.prevline, " " * (40 - len(self.prevline)), (self.key +" "+  key +" " + self.seperator +" " + self.value+" "+str(changedvars[key])))
                    #tempvars[str([*changedvars][0])] = str([*changedvars.values()][0])
                    hasVar = False
                    #for item in 
                    #create a tempvars here and push key,value to tempvars
                    #print("[Debug]", self.prevline, " " * (40 - len(self.prevline)), (self.key +": "+  str([*changedvars][0]) +" " + self.seperator +" " + self.value+": "+str([*changedvars.values()][0])) if len(changedvars) > 0 else "")
                    #print the statement outside of the for loop , fix the multi variable in one line
                elif(hasVar):
